...
describe("Arena", function () {
  ...
  describe(".group()", function () {
    ...
    beforeEach(function () {
      group = arena.group("are-na-team");
    });

    it(".channels() should be a list of aren.na team channels", function () {
      return Promise.all([
        expect(group.channels()).to.eventually.be.an("array"),
        expect(requestHandler).to.have.been.calledWith(
          "get",
          "groups/are-na-team/channels"
        ),
      ]);
    });
  });
  ...
  describe(".block(id)", function () {
    ...
    it(".get() should get the block", function () {
      return Promise.all([
        ...
        expect(requestHandler).to.have.been.calledWith("get", "blocks/8693"),
      ]);
    });

    it(".channels() should get the channels the block belongs to", function () {
      return Promise.all([
  ...
  describe(".user(id)", function () {
    ...
    it(".get() should get the user", function () {
      return Promise.all([
        ...
        expect(requestHandler).to.have.been.calledWith("get", "users/23484"),
      ]);
    });
  });
